...
- **LLM fallback chain**: OpenRouter → NVIDIA NIM. Every call has a timeout and falls through (R8). **No local models** and **no Gemini** — the pipeline's real home is CI, where no daemon is listening, and the project has no Google API access.
